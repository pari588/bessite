...
if (!empty($rawInput) && json_decode($rawInput) === null) {
    ...
    // Legacy fallback key lives in config.inc.php (gitignored) — never hardcode secrets here
    $oldSecurityKey = defined('CAMS_OLD_SECURITY_KEY') ? CAMS_OLD_SECURITY_KEY : '';
...
function processPunchLog($db, $deviceId, $punchLog) {
     ...
     *
     * This is the LIVE CAMS path and it duplicates cams-punch-processor.inc.php,
     * so the same guard has to exist in both files. Keep them in step.
     *
     ...
     * check-OUT with no check-in. Ganesh and Sakshi showed absent for a day
     * they had worked.
     *
     * The raw punch is still stored — never discard evidence — but it is
     * quarantined (status 'Q') and NOT allowed to reach mx_attendance. A wrong
     * attendance row is worse than a missing one: it feeds payroll silently,
     * whereas a missing one gets noticed and corrected.
     */
