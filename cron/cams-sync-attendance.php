...
function syncToHRMSAttendance($db, $camsUserId, $punchDate) {
    ...
    $stmt = $db->prepare(
        'SELECT punch_time FROM camsPunch WHERE user_id = ? AND punch_date_str = ? ORDER BY punch_time ASC'
    );
    ...
    $stmt = $db->prepare('SELECT attendanceID FROM mx_attendance WHERE userID = ? AND attendanceDate = ? LIMIT 1');
