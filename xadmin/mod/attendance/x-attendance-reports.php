...
$DB->vals = array(1);
$DB->types = "i";
$DB->sql = "SELECT userID, employeeCode as empCode, COALESCE(NULLIF(displayName,''), userName) as empName
        ...
        <div class="filters-grid">
            ...
            <div class="filter-group">
                ...
                <select id="filterEmployee" class="filter-select">
                    <option value="">All Employees (Master)</option>
                    <?php foreach ($employees as $emp): ?>
                    <option value="<?php echo $emp['userID']; ?>"><?php echo htmlspecialchars($emp['empCode'] . ' - ' . $emp['empName']); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
...
function generateReport() {
    var params = {
        ...
        employeeID: document.getElementById('filterEmployee').value
    };
...
function renderDailyReport(data) {
    ...
    data.attendance.forEach(function(row) {
        ...
        html += '<td><span class="status-' + (row.status === 'Present' ? 'P' : (row.status === 'Absent' ? 'A' : 'L')) + '">' + row.status + '</span></td>';
        html += '</tr>';
    });
...
function renderMonthlyMuster(data) {
    ...
    data.data.forEach(function(row) {
        ...
        html += '<td class="text-center"><strong>' + row.summary.P + '</strong></td>';
        html += '<td class="text-center">' + row.summary.A + '</td>';
        html += '<td class="text-center">' + row.summary.L + '</td>';
        html += '<td class="text-center">' + row.summary.H + '</td>';
        html += '<td class="text-center">' + row.summary.WO + '</td>';
        html += '<td class="text-center"><strong>' + row.payable + '</strong></td>';
        html += '</tr>';
    });
...
function renderPayrollSummary(data) {
    ...
    data.data.forEach(function(row) {
        ...
        html += '<td class="text-center"><span class="status-L">' + row.leaveDays + '</span></td>';
...
function renderMasterReport(data) {
    ...
    data.data.forEach(function(row) {
        var statusClass = row.status === 'Present' ? 'P' : (row.status === 'Absent' ? 'A' : (row.status === 'Leave' ? 'L' : 'P'));
        html += '<tr>';
        html += '<td>' + row.date + '</td>';
...
function exportReport(format) {
    ...
    params += '&employee=' + document.getElementById('filterEmployee').value;
