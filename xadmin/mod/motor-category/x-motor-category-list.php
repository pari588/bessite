<?php
$arrSearch = array(
    array("type" => "text", "name" => "categoryMID", "title" => "#ID", "where" => "AND categoryMID=?", "dtype" => "i"),
...
<div class="wrap-right">
    ...
    <div class="wrap-data">
        ...
        if ($MXTOTREC > 0) {
            $MXCOLS = array(
                array("#ID", "categoryMID", ' width="1%" align="center"', true),
                array("Image", "imageName", ' width="2%" align="center" nowrap',"","nosort"),
                array("Category Title", "categoryTitle", ' width="33%" align="left"'),
                array("Category URI", "seoUri", ' width="33%" align="left"'),
                array("Display Order", "xOrder", ' width="2%" nowrap align="center"')
            );
            if ($_SESSION[SITEURL]["MXID"] == "SUPER") {
                array_push($MXCOLS, array("Template File", "templateFile", ' align="left"'));
            }

            $arrD = getDepthArray($data, "parentID", "categoryMID");
        ?>
                ...
                <tbody>
                    ...
                    foreach ($arrD as $d) {
                        if ($d["imageName"] != "") {
                            ...
                            $d["imageName"] = getFile(array("path" => "motor_category/" . $arrFile[0], "title" => $d["categoryTitle"]));
                        }
                        if (isset($d["depth"])) {
                    ...
                    ?>
                        <tr>
                            <?php echo getMAction("mid", $d["categoryMID"]); ?>
                            <?php foreach ($MXCOLS as $v) { ?>
                                <td<?php echo $v[2]; ?> title="<?php echo $v[0]; ?>">
                                    <?php if (isset($v[3])&& $v[3] != "") {
                                        echo getViewEditUrl("id=" . $d["categoryMID"], $d[$v[1]], $d["langChild"]);
                                    } else {
                                        echo $d[$v[1]];
                                    } ?>
                                    </td>
