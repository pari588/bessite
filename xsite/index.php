...
if ($TPL->modName == "motors" && $TPL->pageType != "list") {
    ...
    if (!empty($TPL->data) && !empty($TPL->data['motorTitle'])) {
        ...
        // Keyword-optimized meta: model/title + role terms + CG/Crompton brand + Mumbai
        $categoryName = !empty($TPL->dataM['categoryTitle']) ? $TPL->dataM['categoryTitle'] : 'Motor';
        $mt = $TPL->data['motorTitle'];
        $catL = strtolower($categoryName);
        $TPL->metaTitle = $mt . ' – CG Crompton Motor Dealer in Mumbai | Bombay Engineering Syndicate';
        $TPL->metaDesc = $mt . ' from Bombay Engineering Syndicate — authorised CG Power & Crompton Greaves motor dealer, distributor & supplier in Fort, Mumbai. Call +91 93247 06905.';
...
if ($TPL->modName == "motors" && count($TPL->uriArr) == 1 && empty($TPL->metaTitle)) {
    $TPL->metaTitle = 'Industrial Motors Dealer in Mumbai & Ahmedabad | Crompton Authorised | Bombay Engineering Syndicate';
    $TPL->metaDesc = 'Crompton industrial electric motors dealer in Mumbai and Ahmedabad. 84 models across HV/LV, energy-efficient (IE2/IE3/IE4), hazardous-area, FHP and specialty motors. Expert sizing and pan-India delivery since 1957.';
}
...
if ($TPL->modName == "motors" && $TPL->pageType != "x-detail" && !empty($TPL->data) && !empty($TPL->data['categoryTitle'])) {
    $catTitle = $TPL->data['categoryTitle'];
    $catL = strtolower($catTitle);
    $TPL->metaTitle = $catTitle . ' Dealer & Supplier in Mumbai | CG Crompton Authorised | Bombay Engineering Syndicate';
    if (!empty($TPL->data['synopsis'])) {
        $TPL->metaDesc = substr(strip_tags($TPL->data['synopsis']), 0, 160);
    } else {
        $TPL->metaDesc = $catTitle . ' from Bombay Engineering Syndicate — authorised CG Power & Crompton Greaves motor dealer, distributor & supplier in Fort, Mumbai. Genuine products, expert sizing since 1957.';
    }
