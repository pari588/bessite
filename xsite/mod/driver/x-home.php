...
if ($isOffDay) {
    ...
    // 2. There's a cron record that needs Mark Out (recordType=1 with no toTime), OR
    // 3. It's late overtime window AND (no completed record OR can override auto-markout)
    $showMarkOut = $hasOpenShift || $hasCronRecordNeedingMarkOut || ($isLateOvertimeWindow && (!$hasCompletedRecord || $canOverrideAutoMarkout));
