...
$(document).ready(function () {
  webWidth = $('.open-camera').parent('li').width()
 ...
 // Prescription form functionality.
  ...
  $(".button.Front").click(async function () { 
    var configurationStr = $(".button.Front").attr("rel");
...
function configure(facingMode) {
  $('.open-camera').css({'height':webHeight,'width':webWidth})
  Webcam.set({
    width: webWidth,
    height: webHeight,
		dest_width: 576,
...
function take_snapshot() {
  Webcam.snap(function (data_uri) {
    Webcam.reset()
    $('div.open-camera').html(`<img id="imageprev" width="${webWidth}" height="${webHeight}" src="${data_uri}"/>`)
    $('.take-snap').hide()
    PRESPHOTOFLG = 1
  })
}
