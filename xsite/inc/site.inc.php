...
function getSideNav()
{
    ...
    if (is_array($TPL->uriArr) && count($TPL->uriArr) > 0) {
        ...
        // If no category is set (listing page), default to appropriate parent category
        ...
        } elseif ($categoryID == 0 && $TBLCAT == "motor_category") {
            $DB->vals = array(1, "High / Low Voltage AC & DC Motors");
            $DB->types = "is";
            $DB->sql = "SELECT $PKCAT FROM `" . $DB->pre . "$TBLCAT` WHERE status=? AND categoryTitle=?";
            $DB->dbRow();
            if ($DB->numRows > 0) {
    ...
    // ALWAYS show the root category and its complete hierarchy in sidebar
    // This ensures consistent navigation across all product pages
    // Determine the correct parent ID based on table type
    if ($TBLCAT == "pump_category") {
        $sidebarParentID = 1;  // Root pump category
    } elseif ($TBLCAT == "motor_category") {
        $sidebarParentID = 100;  // Root motor category
    } else {
        $sidebarParentID = 1;  // Default fallback
    }
?>
