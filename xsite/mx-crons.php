    ...
    if ($DB->numRows > 0) {
        foreach ($userData as $k => $v) {
            ...
            if ($DB->numRows > 0) {
                ...
                $ch = curl_init();
                $requestParam = "xAction=UPDATE&driverManagementID=" . $driverManagementID . "&toTime=" . urlencode($toTime) . "&fromTime=" . urlencode($fromTime) . "&expenseAmt=" . ($driverManagement['expenseAmt'] ?? 0) . "&dmDate=" . $driverManagement['dmDate'];
