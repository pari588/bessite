...
from nano_banana import image_to_image, NanoBanana
...
os.environ["NANO_BANANA_API_KEY"] = API_KEY
...
def process_image_with_nanobanana(input_path, output_path):
    ...
    try:
        ...
        prompt = """Remove the background completely making it transparent. Enhance the image by:
1. Increasing contrast by 30%
2. Increasing color saturation by 15%
3. Enhancing sharpness
4. Adjusting brightness optimally
Keep the product/motor centered. Return only the enhanced image."""

        # Call image_to_image with correct parameters
        result = image_to_image(
            prompt=prompt,
            reference_images=str(input_path)
        )

        if result and isinstance(result, dict) and 'image_paths' in result:
            ...
            if image_paths:
                ...
                if generated_img.mode == 'RGBA':
                    ...
                    generated_img.convert('RGB').save(output_path, 'WEBP', quality=95, method=6)

                file_size_kb = output_path.stat().st_size / 1024
                log_message(f"✓ Successfully enhanced: {output_path.name} ({file_size_kb:.1f} KB)")
                return True
            else:
                log_message(f"✗ No image in response for {input_path.name}")
                return False
        else:
            log_message(f"✗ Unexpected response format for {input_path.name}: {type(result)}")
            return False
...
def enhance_local_quality(img_path):
    ...
    try:
        ...
        log_message(f"✓ Applied fallback enhancement to {img_path.name}")
        return True

    except Exception as e:
        log_message(f"✗ Fallback enhancement failed: {str(e)}")
        return False
    ...
    try:
        ...
        for idx, image_path in enumerate(images, 1):
            ...
            if process_image_with_nanobanana(image_path, output_path):
                ...
                log_message(f"⚠ Failed, trying fallback enhancement...")
                # Try with just quality enhancement
                try:
                    # Copy original to enhanced folder first
                    from shutil import copy2
                    temp_copy = ENHANCED_DIR / f"{image_path.stem}_temp.webp"
                    img = Image.open(image_path)
                    if img.mode != 'RGB':
                        img = img.convert('RGB')
                    img.save(temp_copy, 'WEBP', quality=90, method=6)
                    if enhance_local_quality(temp_copy):
                        temp_copy.rename(output_path)
                        successful += 1
                    else:
                        failed += 1
                except:
